...
target for functions that use `from mcp_server_for_ynab.server.context import get_app_context`.
"""
...
async def test_missing_plan_id_returns_validation_error(ynab_env: None) -> None:
    ...
    from mcp_server_for_ynab.server.tools.raw.accounts import accounts_list
...
async def test_missing_plan_id_on_enriched_tool_returns_validation_error(ynab_env: None) -> None:
    ...
    from mcp_server_for_ynab.server.tools.enriched import overview_budget_snapshot_tool
...
async def test_auth_failure_returns_structured_error(ynab_env: None) -> None:
    ...
    from mcp_server_for_ynab.server.tools.raw.plans import plans_list

    with patch("mcp_server_for_ynab.server.tools.raw.plans.get_app_context", return_value=mock_ctx):
...
async def test_not_found_returns_structured_error(ynab_env: None) -> None:
    ...
    from mcp_server_for_ynab.server.tools.raw.accounts import accounts_get

    with patch("mcp_server_for_ynab.server.tools.raw.accounts.get_app_context", return_value=mock_ctx):
